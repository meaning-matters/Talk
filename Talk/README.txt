What do we want:
A. Total time in which retries of several servers are done. It's an
   error when this time expires.

B. A maximum retry count for each server per API call. If a server is
   tried this number of times it won't be tried any longer for this
   call. When all server reach this count, it's an error.

C. When a server returns HTTP 4xx this server is no longer retried
   for the current request.  If all server gave a similar error, this
   is shown to the user.

D. When a server returns HTTP 5xx this server is no longer retried
   for the current request.  If all server gave a similar error, this
   is shown to the user.  Code 511 may mean behind captive portal!

E. When the error is lack of internet connection, a server will be
   pushed back in the queue as if it was successful (so low health
   increase). A small delay will be added (e.g. 100ms) before the
   next server will be tried; this to avoid massive amounts of retries
   per second. The per request and per server retry count will not
   be incremented to make sure the full retry delay is consumed.

F. A queue that orders servers on both perceived health and last used.
   After a successful use, a server is pushed back a bit in the queue
   in order to get round-robin behaviour.

   On failure, a server is pushed back a bit further; this keeps it in
   retry/RR scheme, but also gives servers that have proven themselves
   preference. This preference must not be too much because it's unknown
   if a server really failed or if it's the network for example.

G. Each server has an 'order' integer which determins it's place in the
   queue. The head of the queue has the lowest value.

   When a server has been tried, its order is incremented. Then it is put
   at its new place in the queue: After the last item with higher or equal
   order.

   The queue can't be sorted (e.g. with qsort() because servers may have
   the same priority in which case the order of those servers is undetermined
   which would break round-robin. Round-robin is guaranteed by placing a
   server after the last server with same priority.

H. The DNS SRV priority is used. We don't use the lower significant DNS
   SRV weight parameter.

I. Only low DNS SRV priority values should be used: 0, 1, 2, ... (the
   lower the value, the higher the priority).  This number is added to
   the amount a server is pushed back in the queue.

What about an order increment of 1 for a successful call, and an increment
equal to the number of servers in case of failure?



Will your component meet the following (global) requirements:
...
0. START: Record current time and go to 1.

1. Get head of queue. If retryCount > maxRetryCountPerServer
